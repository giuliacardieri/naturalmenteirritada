...
const isHome = computed(() => {
  if (route.name === 'home' || route.name === 'category') return true
  return false
})
...
<template>
  <nav :class="[{ 'header--home': isHome, 'header--closed': !openMenu }, 'header']">
    <h1 v-if="!isHome" class="header__title">
      <RouterLink class="header__title-link" to="/">Naturalmente Irritada</RouterLink>
    </h1>
    ...
    <button class="header__button" aria-label="Abrir ou fechar menu" @click="toggleMenu">
      <svg
        class="header__icon"
        width="48"
        height="48"
